            ...
            <div className="p-6 space-y-6">
              <div className="flex items-center gap-4 mb-6">
                <div className="w-12 h-12 rounded-full bg-brand-primary/10 text-brand-primary flex items-center justify-center font-bold text-lg">
                  {(Array.isArray(caseData.profiles) ? (caseData.profiles[0] as {name: string})?.name?.[0] : (caseData.profiles as {name: string})?.name?.[0])?.toUpperCase() || 'C'}
                </div>
                <div>
                  <div className="font-bold text-slate-900">{(Array.isArray(caseData.profiles) ? (caseData.profiles[0] as {name: string})?.name : (caseData.profiles as {name: string})?.name)}</div>
                  <div className="text-xs text-slate-500">{(Array.isArray(caseData.profiles) ? (caseData.profiles[0] as {email: string})?.email : (caseData.profiles as {email: string})?.email)}</div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-start gap-3 text-sm">
                  ...
                  <span className="text-slate-600">{(Array.isArray(caseData.profiles) ? (caseData.profiles[0] as {phone: string})?.phone : (caseData.profiles as {phone: string})?.phone) || 'Não informado'}</span>
                </div>
                <div className="flex items-start gap-3 text-sm">
